...
public class TransporterPort implements TransporterPortType{
	...
	public JobView requestJob(String origin, String destination, int price)
		...
		job.setJobIdentifier(Integer.toString(var));
		var++;
		...
		if(price <= 0){
			...
			if(price > 100){
				...
				valor = ThreadLocalRandom.current().nextInt(1, price-1);
			}
			else{
				if(price%2==1){
					if(transporter%2 == 1){
	...
	public JobView decideJob(String id, boolean accept) throws BadJobFault_Exception {
		...
		for(int i=0; i<listJob.size(); i++){
			if(listJob.get(i).getJobIdentifier().equals(id)){
				a = listJob.get(i).getJobState();
				if(a == JobStateView.ACCEPTED || a == JobStateView.COMPLETED){
					BadJobFault faultInfo = new BadJobFault();
					faultInfo.setId(id);
					throw new BadJobFault_Exception("error in server", faultInfo);
				}
				else if(a == JobStateView.REJECTED){
					BadJobFault faultInfo = new BadJobFault();
					faultInfo.setId(id);
					throw new BadJobFault_Exception("error in server", faultInfo);
